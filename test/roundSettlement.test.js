...
const makeHand = ({ cards, bet, sb = { pp: 0, t: 0 }, result = null, message = "", payout = 0, done = false, doubled = false, insuranceTaken = false, insuranceAmount = 0 }) => ({
  cards,
  bet,
  sb,
  result,
...
test("insurance is half of the total main wager", () => {
  ...
  assert.equal(getInsuranceAmountForHand(1_000_000), 500_000);
});

test("round summary counts successful insurance calls as wins before losses", () => {
  const summary = summarizeRoundResults([
    makeHand({
      cards: [makeCard("9", "hearts"), makeCard("7", "clubs")],
      bet: 1_000_000,
      result: "lose",
      insuranceTaken: true,
      insuranceAmount: 500_000,
    }),
    makeHand({
      cards: [makeCard("10", "hearts"), makeCard("6", "clubs")],
      bet: 1_000_000,
      result: "lose",
      insuranceTaken: true,
      insuranceAmount: 500_000,
    ...
    makeHand({
      cards: [makeCard("9", "spades"), makeCard("7", "diamonds")],
      bet: 1_000_000,
      result: "lose",
    }),
    makeHand({
      cards: [makeCard("8", "spades"), makeCard("8", "diamonds")],
      bet: 1_000_000,
      result: "lose",
    }),
  ], true);

  assert.deepEqual(summary, { wins: 2, losses: 2, pushes: 0 });
});
...
test("immediate settlement pays blackjack at 7:5", () => {
  const hands = [
    makeHand({
  ...
  assert.equal(settled.hands[0].payout, 2_400_000);
  assert.equal(settled.immediatePayout, 2_400_000);
});
