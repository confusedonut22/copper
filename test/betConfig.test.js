...
test("normalizeStakeConfig accepts minStep as a stepBet alias", () => {
  const config = normalizeStakeConfig({
    minBet: 100_000,
    maxBet: 500_000,
    minStep: 50_000,
    sideBets: {
      pp: { minBet: 50_000, maxBet: 250_000, minStep: 50_000 },
    },
  });

  assert.equal(config.stepBet, 50_000);
  assert.equal(config.sideBets.pp.stepBet, 50_000);
  assert.equal(isAllowedStakeBet(150_000, config), true);
  assert.equal(isAllowedStakeBet(175_000, config), false);
  assert.equal(isAllowedStakeSideBet(100_000, "pp", config), true);
  assert.equal(isAllowedStakeSideBet(125_000, "pp", config), false);
});
