...
import { stateLayoutDerived } from "./layout.svelte.js";
...
function isMobileDeviceHint() {
  if (typeof window === "undefined") return false;
  return new URLSearchParams(window.location.search).get("device") === "mobile";
}

function getStageMetrics(width, height) {
  const canvasSizes = stateLayoutDerived.canvasSizes();
  const mainLayout = stateLayoutDerived.mainLayout();
  const fallbackX = width / 2;
  const fallbackDealerY = 20;
  const fallbackPlayerY = height * 0.45;
  const mainWidth = mainLayout?.width ?? width;
  const mainHeight = mainLayout?.height ?? height;
  const mainScale = mainLayout?.scale ?? 1;
  const mainX = mainLayout?.x ?? canvasSizes.width / 2 ?? fallbackX;
  const mainY = mainLayout?.y ?? canvasSizes.height / 2 ?? height / 2;
  const topEdge = mainY - (mainHeight * mainScale) / 2;

  return {
    mainX: Number.isFinite(mainX) ? mainX : fallbackX,
    dealerY: Number.isFinite(topEdge) ? topEdge + mainHeight * 0.0185 * mainScale : fallbackDealerY,
    playerY: Number.isFinite(topEdge)
      ? topEdge + mainHeight * (stateLayoutDerived.isStacked() ? 0.58 : 0.45) * mainScale
      : fallbackPlayerY,
  };
}

function applyStageLayout(width, height) {
  const { mainX, dealerY, playerY } = getStageMetrics(width, height);
  if (dealerContainer) {
    dealerContainer.x = mainX;
    dealerContainer.y = dealerY;
  }
  if (cardContainer) {
    cardContainer.x = mainX;
    cardContainer.y = playerY;
  }
  if (stageLabel) {
    stageLabel.x = mainX;
    stageLabel.y = height * 0.52;
  }
}
...
export async function initCanvas(canvasEl, width, height) {
  ...
  await app.init({
    ...
    antialias: true,
    resolution: Math.min(window.devicePixelRatio || 1, isMobileDeviceHint() ? 1.5 : 2),
    autoDensity: true,
  });
...
function drawFelt(stage, w, h) {
  ...
  label.x = stateLayoutDerived.mainLayout().x;
  label.y = h * 0.52;
  label.alpha = 0.35;
  stageLabel = label;
