...
export function getInsuranceAmountForHand(handBet) {
  return Math.floor((handBet ?? 0) / 2);
}
...
export function summarizeRoundResults(hands, dealerHasBlackjack = false) {
  let wins = 0;
  let losses = 0;
  let pushes = 0;

  for (const hand of hands) {
    const insuranceWin = dealerHasBlackjack && (hand.insuranceTaken === true) && (hand.insuranceAmount ?? 0) > 0;
    if (insuranceWin) {
      wins += 1;
      continue;
    }
    if (hand.result === "win" || hand.result === "blackjack") wins += 1;
    else if (hand.result === "lose" || hand.result === "bust") losses += 1;
    else if (hand.result === "push") pushes += 1;
  }
