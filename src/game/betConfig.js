...
function normalizeSideBetConfig(rawConfig) {
  ...
  const normalizedStepBet = toInt(rawConfig.stepBet) ?? toInt(rawConfig.minStep);
  const normalized = {
    minBet: toInt(rawConfig.minBet),
    maxBet: toInt(rawConfig.maxBet),
    stepBet: normalizedStepBet,
...
export function normalizeStakeConfig(config = {}) {
  const normalizedStepBet = toInt(config.stepBet) ?? toInt(config.minStep);
  const normalized = {
