...
export function hydrateStakeRound(round) {
  ...
  const restoredPending = state.pendingInsurance
    ? {
        ...
        insuranceByHand: state.pendingInsurance.insuranceByHand ?? restoredHands.map(() => 0),
      }
    : null;
...
export async function deal() {
  ...
  if (hasStakeSession()) {
    try {
      ...
      rgsError.set("Stake session did not return a compatible round.state payload.");
      return;
    } catch (error) {
      rgsStatus.set("play-error");
      rgsError.set(error instanceof Error ? error.message : String(error));
  ...
  setTimeout(() => {
    ...
    const insuranceByHand = freshHands.map((hand) => getInsuranceAmountForHand(hand.bet));
    const insuranceAmount = getInsuranceAmount($mt);

    // Insurance prompt
    if (dealerCards[0].rank === "A" && !$auto) {
      pending.set({ dealerCards, freshHands, dealerBJ, insuranceAmount, insuranceByHand });
      phase.set(PHASE.INS);
    } else {
      resolveInitial(dealerCards, freshHands, {
        accepted: false,
        amount: 0,
        insuranceByHand: freshHands.map(() => 0),
      });
    }
  }, $auto ? sp.deal : 350);
}

export function takeInsurance(take, customAmount = null, selectedHandIndexes = null) {
  if (get(replayMode)) return;
  const $pend = get(pending);
  if (!$pend) return;
  const selectedIndexes = Array.isArray(selectedHandIndexes)
    ? selectedHandIndexes.filter((index) => Number.isInteger(index) && index >= 0 && index < $pend.freshHands.length)
    : $pend.freshHands.map((_, index) => index);
  const insuranceByHand = $pend.freshHands.map((_, index) =>
    take && selectedIndexes.includes(index) ? ($pend.insuranceByHand?.[index] ?? getInsuranceAmountForHand($pend.freshHands[index]?.bet)) : 0,
  );
  const effectiveAmount = customAmount !== null ? customAmount : insuranceByHand.reduce((sum, amount) => sum + amount, 0);
  if (hasStakeSession()) {
    pending.set(null);
    emitRoundEvent({
      type: "insuranceDecision",
      accepted: Boolean(take) && effectiveAmount > 0,
      amount: take ? effectiveAmount : 0,
      hands: insuranceByHand
        .map((amount, handIndex) => amount > 0 ? { handIndex, amount } : null)
        .filter(Boolean),
    }).then(() => {
  ...
  emitRoundEvent({
    ...
    hands: insuranceByHand
      .map((amount, handIndex) => accepted && amount > 0 ? { handIndex, amount } : null)
      .filter(Boolean),
  });
  resolveInitial(
    $pend.dealerCards,
    $pend.freshHands,
    {
      accepted,
      amount: accepted ? effectiveAmount : 0,
      insuranceByHand: accepted ? insuranceByHand : $pend.freshHands.map(() => 0),
    }
  );
}

function resolveInitial(dealerCards, hs, insuranceSelection) {
  const {
    hands: updated,
    sideBetPayout,
    immediatePayout,
  ...
  const insuranceByHand = insuranceSelection?.insuranceByHand ?? updated.map(() => 0);
  const insuredHands = updated.map((hand, index) => ({
    ...hand,
    insuranceTaken: (insuranceByHand[index] ?? 0) > 0,
    insuranceAmount: insuranceByHand[index] ?? 0,
  }));

  if (dealerBJ && (insuranceSelection?.amount ?? 0) > 0) {
    balance.update(b => b + ((insuranceSelection.amount ?? 0) * 3));
  }
  ...
  emitRoundEvent({
    ...
    insuranceTaken: insuranceByHand.some((amount) => amount > 0),
    insuranceAmount: insuranceSelection?.amount ?? 0,
    insuranceByHand,
    immediatePayout,
    hands: insuredHands.map((hand, handIndex) => ({
      handIndex,
      result: hand.result,
      payout: hand.payout,
      done: hand.done,
  ...
  hands.set(insuredHands);

  const nx = findNextActive(insuredHands);
  if (nx >= 0) {
    activeHand.set(nx);
    phase.set(PHASE.PLAY);
  } else {
    finishRound(insuredHands, handValue(dealerCards), dealerBJ);
  }
}
...
export function split() {
  ...
  if (h.isSplit || h.isAceSplit) return;
  if ($hands.length >= SPLIT_HAND_LIMIT) return;
  if ($balance < h.bet) return;

  if (!isSplittablePair(h.cards)) return;
  ...
  if (hasStakeSession()) {
    ...
    emitRoundEvent({
      ...
      const hydrated = Boolean(response?.parsedEvent?.state && canHydrateRoundState(response.parsedEvent.state));
      const hydratedHands = get(hands);
      const hydratedHand = hydratedHands[$actH];
      const splitApplied =
        hydratedHands.length > $hands.length
        || hydratedHand?.isSplit === true
        || hydratedHand?.cards?.length === 1;
      if (!hydrated || !splitApplied) {
        applyLocalSplit($actH, { playSnap: false, emitEvent: false });
      }
      if (get(phase) === PHASE.RESULT) requestEndRound();
    });
    return;
  ...
  applyLocalSplit($actH, { playSnap: true, emitEvent: true });
}

function applyLocalSplit(handIndex, { playSnap = true, emitEvent = true } = {}) {
  const $hands = get(hands);
  const $balance = get(balance);
  const h = $hands[handIndex];
  if (!h || h.cards.length !== 2) return false;
  if (h.isSplit || h.isAceSplit) return false;
  if ($hands.length >= SPLIT_HAND_LIMIT) return false;
  if ($balance < h.bet) return false;
  if (!isSplittablePair(h.cards)) return false;
  ...
  hands.update(hs => {
    const updated = [...hs];
    updated[handIndex] = hand1;
    updated.splice(handIndex + 1, 0, hand2);
    return updated;
  });
...
function finishRound(hs, dealerVal, dealerHasBlackjack = false) {
  const { wins, losses, pushes } = summarizeRoundResults(hs, dealerHasBlackjack);
